...
#define XMODEL_PATH                     "test_3_compiled.xmodel"

#define TEST_C1_RESULT_CSV_PATH         "test_C1_kv260_results_cpp.csv"
#define TEST_C1_POWER_RESULT_CSV_PATH   "test_C1_kv260_power_results_cpp.csv"

#define TEST_C2_RESULT_CSV_PATH         "test_C2_kv260_results_cpp.csv"
#define TEST_C2_POWER_RESULT_CSV_PATH   "test_C2_kv260_power_results_cpp.csv"
 ...
 */
void TestC1(const std::vector<string>& image_paths, 
            ...
            const std::string& results_csv)
{
    ...
    if (subgraphs.empty()) 
    {
        throw runtime_error("No DPU subgraph found in " XMODEL_PATH);
    }

    if (subgraphs.size() < 2) 
    {
        throw std::runtime_error("Need at least 2 DPU subgraphs for TestC1");
    }

    //auto runner_1 = vart::Runner::create_runner(subgraphs[0], "run");
    //auto runner_2 = vart::Runner::create_runner(subgraphs[1], "run");

    auto runner_1 = vart::Runner::create_runner(subgraphs[1], "run");   // resnet50 on subgraphs 1
    auto runner_2 = vart::Runner::create_runner(subgraphs[0], "run");   // vgg16 on subgraphs 0
 ...
 */
void TestC2(const std::vector<string>& image_paths,
            const std::vector<string>& labels,
            const std::unordered_map<std::string, int>& val_mapping,
            const std::string& results_csv)
{
    ...
    if (subgraphs.size() < 2) 
    {
        throw runtime_error("Need at least 2 DPU subgraphs for TestC2");
    }
...
{
    // Start power logger
    //Start_kv260_power_log(TEST_C1_POWER_RESULT_CSV_PATH);
    Start_kv260_power_log(TEST_C2_POWER_RESULT_CSV_PATH);
