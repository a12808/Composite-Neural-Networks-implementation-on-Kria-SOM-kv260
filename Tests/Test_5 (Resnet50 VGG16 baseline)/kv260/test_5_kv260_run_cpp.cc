...
#define RESNET50_0_XMODEL_PATH          "resnet50_0_compiled.xmodel"
#define RESNET50_1_XMODEL_PATH          "resnet50_1_compiled.xmodel"
#define VGG16_XMODEL_PATH               "vgg16_compiled.xmodel"

#define TEST_A0_RESULT_CSV_PATH         "test_A0_kv260_results_cpp.csv"
#define TEST_A0_POWER_RESULT_CSV_PATH   "test_A0_kv260_power_results_cpp.csv"

#define TEST_A1_RESULT_CSV_PATH         "test_A1_kv260_results_cpp.csv"
#define TEST_A1_POWER_RESULT_CSV_PATH   "test_A1_kv260_power_results_cpp.csv"

#define TEST_B0_RESULT_CSV_PATH         "test_B0_kv260_results_cpp.csv"
#define TEST_B0_POWER_RESULT_CSV_PATH   "test_B0_kv260_power_results_cpp.csv"

#define TEST_C0_RESULT_CSV_PATH         "test_C0_kv260_results_cpp.csv"
#define TEST_C0_POWER_RESULT_CSV_PATH   "test_C0_kv260_power_results_cpp.csv"

// power thread
#define SYSFS_POWER_PATH                "/sys/class/hwmon/hwmon0/power1_input"
#define POWER_LOG_INTERVAL_S            0.1
 ...
 * code recycling
 */
void Run_single_inference(const cv::Mat& img,
                          vart::Runner* runner,
                          ...
                          double& postprocess_time)
{
    ...
    preprocess_time = get_timestamp() - t_start;
    ...
    for (auto& input_buffer : input_tensor_buffers) 
    {
    ...
    upload_time = get_timestamp() - t_start;
    ...
    auto run_inference = [](vart::Runner* runner,
                            const std::vector<vart::TensorBuffer*>& in_bufs,
                            const std::vector<vart::TensorBuffer*>& out_bufs) {
        auto job_id = runner->execute_async(in_bufs, out_bufs);
        runner->wait(job_id.first, -1);
    };
    
    std::thread t(run_inference, runner, input_tensor_buffers, output_tensor_buffers);
    t.join();
    
    inference_time = get_timestamp() - t_start;
    ...
    download_time = get_timestamp() - t_start;

    // -------------------------------
    // POSTPROCESS
    // -------------------------------
    t_start = get_timestamp();
    for (size_t i = 0; i < output_tensor_buffers.size(); ++i) 
    {     
        std::vector<float> logits;
        
        try 
        {
            int fp = std::any_cast<int>(output_tensors[i]->get_attr("fix_point"));
            float scale = std::pow(2.0f, -fp);
            auto tensor = output_tensor_buffers[i]->get_tensor();
            int size = tensor->get_element_num();
            logits.resize(size);

            auto data_ptr = reinterpret_cast<int8_t*>(output_tensor_buffers[i]->data().first);
        ...
        auto [cid, p] = Postprocess_top1(logits);
    ...
    if (pred_lbl >= 0 && pred_lbl < labels.size()) 
    {
        label_name = labels[pred_lbl];
    }

    postprocess_time = get_timestamp() - t_start;
}

/**
 * TEST A0 A1
 * baseline single compiled model inference 
 */
void Test_Baseline(const std::string& compiled_model_path,
    const std::vector<string>& image_paths,
    ...
    const std::string& results_csv)
{
    ...
    file << "img_name,"
         ...
         << "gt_lbl,gt_name,"
         << "load_time_1,preprocess_time_1,upload_time_1,inference_time_1,download_time_1,postprocess_time_1,"
         << "total_time\n";

    // -------------------------------
    // MODEL LOAD
    // -------------------------------
    auto graph1 = xir::Graph::deserialize(compiled_model_path);
    auto root1 = graph1->get_root_subgraph();
    auto child_subgraphs1 = root1->children_topological_sort();
    
    // non-owning pointers !!!
    std::vector<xir::Subgraph*> subgraphs1;

    for (auto& sg : child_subgraphs1) 
    {
        if (sg->has_attr("device")) 
        {
            auto device = sg->get_attr<std::string>("device");
            transform(device.begin(), device.end(), device.begin(), ::toupper);
            if (device == "DPU") subgraphs1.push_back(sg); // store raw pointer, do NOT own it
        }
    }

    if (subgraphs1.empty())
    {
        throw std::runtime_error("No DPU subgraph found in " + compiled_model_path);
    }

    auto runner_1 = vart::Runner::create_runner(subgraphs1[0], "run");

    if (!runner_1) 
    {
        throw std::runtime_error("Failed to create runner_0");
    }

    debug_subgraphs(subgraphs1);
    ...
    for (size_t i = 0; i < image_paths.size(); ++i)
    {
        ...
        double preprocess_time_1, upload_time_1, inference_time_1, download_time_1, postprocess_time_1;
        
        Run_single_inference(img, runner_1.get(), labels, pred_lbl_1, prob_1, label_name_1, preprocess_time_1, upload_time_1, inference_time_1, download_time_1, postprocess_time_1);
    ...
    runner_1.reset();   // free runner resources
    graph1.reset();     // free graph resources
    file.close();
}

/**
 * TEST B0 C0
 * Two sequencial compiled models inference with max cost
 */
void Test_Sequencial(const std::string& compiled_model_1_path,
    const std::string& compiled_model_2_path,
    const std::vector<string>& image_paths,
    const std::vector<string>& labels,
    const std::unordered_map<std::string, int>& val_mapping,
    const std::string& results_csv)
{
    ...
    file << "img_name,"
         ...
         << "gt_lbl,gt_name,"
         << "load_time_1,model_load_time_1,preprocess_time_1,upload_time_1,inference_time_1,download_time_1,postprocess_time_1,"
         << "load_time_2,model_load_time_2,preprocess_time_2,upload_time_2,inference_time_2,download_time_2,postprocess_time_2,"
         << "total_time\n";
    ...
    for (size_t i = 0; i < image_paths.size(); ++i)
    {
        ...
        auto graph1 = xir::Graph::deserialize(compiled_model_1_path);
        auto root1 = graph1->get_root_subgraph();
        auto child_subgraphs1 = root1->children_topological_sort();
        ...
        if (subgraphs1.empty()) 
        {
            throw std::runtime_error("No DPU subgraph found in " + compiled_model_1_path);
        }
        ...
        double model_load_time_1 = get_timestamp() - t_start;
        ...
        Run_single_inference(img_1, runner_1.get(), labels, pred_lbl_1, prob_1, label_name_1, preprocess_time_1, upload_time_1, inference_time_1, download_time_1, postprocess_time_1);
        ...
        auto graph2 = xir::Graph::deserialize(compiled_model_2_path);
        auto root2 = graph2->get_root_subgraph();
        auto child_subgraphs2 = root2->children_topological_sort();
        
        // non-owning pointers !!!
        std::vector<xir::Subgraph*> subgraphs2;
        ...
        for (auto& sg : child_subgraphs1) 
        {
            if (sg->has_attr("device")) 
            {
                auto device = sg->get_attr<std::string>("device");
                transform(device.begin(), device.end(), device.begin(), ::toupper);
                if (device == "DPU") subgraphs2.push_back(sg); // store raw pointer, do NOT own it
            }
        }

        if (subgraphs2.empty())
        { 
            throw std::runtime_error("No DPU subgraph found in " + compiled_model_2_path);
        }

        auto runner_2 = vart::Runner::create_runner(subgraphs2[0], "run");
        
        if (!runner_2) 
        {
            throw std::runtime_error("Failed to create runner_2");
        }

        double model_load_time_2 = get_timestamp() - t_start;


        int pred_lbl_2 = -1;
        float prob_2 = 0.0f;
        std::string label_name_2 = "UNK";
        double preprocess_time_2, upload_time_2, inference_time_2, download_time_2, postprocess_time_2;
        
        Run_single_inference(img_2, runner_2.get(), labels, pred_lbl_2, prob_2, label_name_2, preprocess_time_2, upload_time_2, inference_time_2, download_time_2, postprocess_time_2);


        runner_1.reset();   // free runner resources
        graph1.reset();     // free graph resources
        runner_2.reset();   // free runner resources
        graph2.reset();     // free graph resources
        ...
        file << img_name << ","
            << pred_lbl_1 << "," << "\"" << label_name_1 << "\"" << "," << prob_1 << ","
            << pred_lbl_1 << "," << "\"" << label_name_2 << "\"" << "," << prob_2 << ","
            << gt_lbl << "," << "\"" << gt_name    << "\"" << ","
            << load_time_1 << "," << model_load_time_1 << "," << preprocess_time_1 << "," << upload_time_1 << "," << inference_time_1 << "," << download_time_1 << "," << postprocess_time_1 << ","
            << load_time_2 << "," << model_load_time_2 << "," << preprocess_time_2 << "," << upload_time_2 << "," << inference_time_2 << "," << download_time_2 << "," << postprocess_time_2 << ","
            << total_time << "\n";
...
{
    // Start power logger
    //Start_kv260_power_log(TEST_A0_POWER_RESULT_CSV_PATH);
    //Start_kv260_power_log(TEST_A1_POWER_RESULT_CSV_PATH);
    Start_kv260_power_log(TEST_B0_POWER_RESULT_CSV_PATH);
    //Start_kv260_power_log(TEST_C0_POWER_RESULT_CSV_PATH);
    ...
    // TestA0
    //Test_Baseline(RESNET50_0_XMODEL_PATH, image_files, labels, val_mapping, TEST_A0_RESULT_CSV_PATH);
    
    // TestA1
    //Test_Baseline(VGG16_XMODEL_PATH, image_files, labels, val_mapping, TEST_A1_RESULT_CSV_PATH);
    
    // TestB0
    Test_Sequencial(RESNET50_0_XMODEL_PATH, RESNET50_1_XMODEL_PATH, image_files, labels, val_mapping, TEST_B0_RESULT_CSV_PATH);
    
    // TestC0
    //Test_Sequencial(RESNET50_0_XMODEL_PATH, VGG16_XMODEL_PATH, image_files, labels, val_mapping, TEST_C0_RESULT_CSV_PATH);
