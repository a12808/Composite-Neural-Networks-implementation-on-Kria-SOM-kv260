...
class Config:
    ...
    def __init__(self):
        ...
        self.fused_model_path = f"{self.project_path}/test_4_fused_model.pt"
        self.fused_model_onnx_path = f"{self.project_path}/test_4_fused_model.onnx"
        ...
        # deploy source path
        self.compiled_fused_model_path = f"{self.compilation_output_dir}/test_4_compiled.xmodel"
        self.run_script_path = f"{self.project_path}/test_4_kv260_run.py"

        # Deploy directories
        self.deploy_dir = f"{self.project_path}/{self.project_name}"

        # Deploy destination path
        self.deploy_model_dest = f"{self.deploy_dir}/test_4_compiled.xmodel"
        self.deploy_run_script_dest = f"{self.deploy_dir}/test_4_kv260_run.py"
...
class ResnetFusionSaver:
    def __init__(self, output_path_pt, output_path_onnx):
        super().__init__()
        fusion_model = ResnetFusionModel()
...
class ResnetFusionLoader(nn.Module):
    ...
    def __init__(self, fusion_model_path, device='cpu'):
        super().__init__()
        self.device = device
        self.model = torch.load(fusion_model_path, map_location=device)
        self.model.eval()
...
def test_model_predictions(model, img1_tensor, img2_tensor, labels):
    ...
    with torch.no_grad():
        ...
        # divide o vetor fundido em duas partes
        out1 = output[:, :1000]   # predições da model1
        out2 = output[:, 1000:]   # predições da model2

        # pega o índice da classe de maior probabilidade
...
def compile_for_kv260(config):
    ...
    compile_cmd = f"vai_c_xir -x {xmodel_path} -a {arch_json_path} -o {config.compilation_output_dir} -n test_4_compiled"
    print("Executing command: ", f"{compile_cmd}")
    os.system(compile_cmd)
    
    # Verify DPU subgraphs
    verify_cmd = f"xir png {config.compilation_output_dir}/test_4_compiled.xmodel {config.compilation_output_dir}/test_4_subgraphs.png"
