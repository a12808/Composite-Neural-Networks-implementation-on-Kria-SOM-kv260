...
class Config:
    ...
    def __init__(self):
        ...
        self.fused_model_path = f"{self.project_path}/test_1_fused_model.pt"
        self.fused_model_onnx_path = f"{self.project_path}/test_1_fused_model.onnx"
        ...
        # deploy source path
        self.compiled_fused_model_path = f"{self.compilation_output_dir}/test_1_compiled.xmodel"
        self.run_script_path = f"{self.project_path}/test_1_kv260_run.py"

        # Deploy directories
        self.deploy_dir = f"{self.project_path}/{self.project_name}"

        # Deploy destination path
        self.deploy_model_dest = f"{self.deploy_dir}/test_1_compiled.xmodel"
        self.deploy_run_script_dest = f"{self.deploy_dir}/test_1_kv260_run.py"
...
def compile_for_kv260(config):
    ...
    compile_cmd = f"vai_c_xir -x {xmodel_path} -a {arch_json_path} -o {config.compilation_output_dir} -n test_1_compiled"
    print("Executing command: ", f"{compile_cmd}")
    os.system(compile_cmd)
    
    # Verify DPU subgraphs
    verify_cmd = f"xir png {config.compilation_output_dir}/test_1_compiled.xmodel {config.compilation_output_dir}/test_1_subgraphs.png"
