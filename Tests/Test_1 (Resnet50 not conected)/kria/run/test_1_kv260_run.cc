...
#define XMODEL_PATH                     "test_1_compiled.xmodel"

#define TEST_B1_RESULT_CSV_PATH         "test_B1_kv260_results_cpp.csv"
#define TEST_B1_POWER_RESULT_CSV_PATH   "test_B1_kv260_power_results_cpp.csv"

#define TEST_B2_RESULT_CSV_PATH         "test_B2_kv260_results_cpp.csv"
#define TEST_B2_POWER_RESULT_CSV_PATH   "test_B2_kv260_power_results_cpp.csv"
...
/**
 * Two "paralel" inference
 */
void TestB1(const std::vector<string>& image_paths, 
            ...
            const std::string& results_csv)
{
    ...
    if (subgraphs.size() < 2) 
    {
        throw std::runtime_error("Need at least 2 DPU subgraphs for TestB1");
    }

    auto runner_1 = vart::Runner::create_runner(subgraphs[0], "run");
    auto runner_2 = vart::Runner::create_runner(subgraphs[1], "run");

    if (!runner_1) 
    {
        throw std::runtime_error("Failed to create runner_1");
    }
    ...
    for (size_t i = 0; i < image_paths.size(); ++i) 
    {
        ...
        // -------------------------------
        // PREPROCESS DPU BUFFERS 1 2
        // -------------------------------
        t_start = get_timestamp();

        // Obter os descritores de input/output
        auto input_tensors_1    = runner_1->get_input_tensors();
        auto output_tensors_1   = runner_1->get_output_tensors();
        
        // Criar buffers para inputs/outputs
        std::vector<std::unique_ptr<vart::TensorBuffer>> input_tensor_buffers_owner_1;
        std::vector<std::unique_ptr<vart::TensorBuffer>> output_tensor_buffers_owner_1;

        std::vector<vart::TensorBuffer*> input_tensor_buffers_1;
        std::vector<vart::TensorBuffer*> output_tensor_buffers_1;

        // Alocar buffers de input
        for (auto& tensor : input_tensors_1) 
        {
            auto buffer = vart::alloc_cpu_flat_tensor_buffer(tensor);
            input_tensor_buffers_owner_1.push_back(std::move(buffer));
            input_tensor_buffers_1.push_back(input_tensor_buffers_owner_1.back().get());
        }

        // Alocar buffers de output  
        for (auto& tensor : output_tensors_1) {

            auto buffer = vart::alloc_cpu_flat_tensor_buffer(tensor);
            output_tensor_buffers_owner_1.push_back(std::move(buffer));
            output_tensor_buffers_1.push_back(output_tensor_buffers_owner_1.back().get());
        }
        ...
        auto input_tensors_2    = runner_2->get_input_tensors();
        auto output_tensors_2   = runner_2->get_output_tensors();

        // Criar buffers para inputs/outputs
        std::vector<std::unique_ptr<vart::TensorBuffer>> input_tensor_buffers_owner_2;
        std::vector<std::unique_ptr<vart::TensorBuffer>> output_tensor_buffers_owner_2;

        std::vector<vart::TensorBuffer*> input_tensor_buffers_2;
        std::vector<vart::TensorBuffer*> output_tensor_buffers_2;


        // Alocar buffers de input
        for (auto& tensor : input_tensors_2) 
        {
            auto buffer = vart::alloc_cpu_flat_tensor_buffer(tensor);
            input_tensor_buffers_owner_2.push_back(std::move(buffer));
            input_tensor_buffers_2.push_back(input_tensor_buffers_owner_2.back().get());
        }

        // Alocar buffers de output  
        for (auto& tensor : output_tensors_2) {

            auto buffer = vart::alloc_cpu_flat_tensor_buffer(tensor);
            output_tensor_buffers_owner_2.push_back(std::move(buffer));
            output_tensor_buffers_2.push_back(output_tensor_buffers_owner_2.back().get());
        }
        ...
        // Para cada buffer de input, copiar dados
        for (auto& input_buffer : input_tensor_buffers_1) 
        {
            // Fazer sync para escrita
            input_buffer->sync_for_write(0, input_buffer->get_tensor()->get_data_size());
            
            // Obter apontador para os dados
            auto data_ptr = input_buffer->data().first;
            size_t buffer_size = input_buffer->get_tensor()->get_data_size();
            
            // Copiar dados da imagem processada
            std::memcpy((void*)data_ptr, blob_1.data, std::min(buffer_size, (size_t)(blob_2.total() * blob_2.elemSize())));
        }

        // sync input tensor buffers
        for (auto& input : input_tensor_buffers_1) 
        {
            input->sync_for_write(0, input->get_tensor()->get_data_size() / input->get_tensor()->get_shape()[0]);
        }

        double upload_time_1 = get_timestamp() - t_start;

        t_start = get_timestamp();

        for (auto& input_buffer : input_tensor_buffers_2) 
        {
            // Fazer sync para escrita
            input_buffer->sync_for_write(0, input_buffer->get_tensor()->get_data_size());
            ...
            // Copiar dados da imagem processada
            // NOTA: Assume-se que o pré-processamento já formata os dados corretamente
            std::memcpy((void*)data_ptr, blob_2.data, std::min(buffer_size, (size_t)(blob_2.total() * blob_2.elemSize())));
        }

        // sync input tensor buffers
        for (auto& input : input_tensor_buffers_2) 
        {
            input->sync_for_write(0, input->get_tensor()->get_data_size() / input->get_tensor()->get_shape()[0]);
        }

        double upload_time_2 = get_timestamp() - t_start;
        ...
        std::thread t1(run_inference, runner_1.get(), input_tensor_buffers_1, output_tensor_buffers_1);
        std::thread t2(run_inference, runner_2.get(), input_tensor_buffers_2, output_tensor_buffers_2);
        t1.join();
        t2.join();
        ...
        for (auto& output_buffer : output_tensor_buffers_1) 
        {
            output_buffer->sync_for_read(0, output_buffer->get_tensor()->get_data_size());
        }

        double download_time_1 = get_timestamp() - t_start;

        t_start = get_timestamp();

        for (auto& output_buffer : output_tensor_buffers_2) 
        {
            output_buffer->sync_for_read(0, output_buffer->get_tensor()->get_data_size());
        }

        double download_time_2 = get_timestamp() - t_start;
        ...
        for (size_t i = 0; i < output_tensor_buffers_1.size(); ++i) 
        {
            std::vector<float> logits;

            try 
            {
                int fp = std::any_cast<int>(output_tensors_1[i]->get_attr("fix_point"));
                float scale = std::pow(2.0f, -fp);

                auto tensor = output_tensor_buffers_1[i]->get_tensor();
                int size = tensor->get_element_num();
                logits.resize(size);

                auto data_ptr = reinterpret_cast<int8_t*>(output_tensor_buffers_1[i]->data().first);
            ...
            // Processar as logits para obter a classe predita
            auto [predicted_cid_1, predicted_prob_1] = postprocess_top1(logits);

            // Atualizar apenas se esta for a predição principal ou se for o primeiro output
            if (i == 0 || predicted_prob_1 > prob_1) 
            {
                pred_lbl_1 = predicted_cid_1;
                prob_1 = predicted_prob_1;
            }  
        }

        // Obter o nome da label
        if (pred_lbl_1 >= 0 && pred_lbl_1 < labels.size()) 
        {
            label_name_1 = labels[pred_lbl_1];
        }
        
        double postprocess_time_1 = get_timestamp() - t_start;
        ...
        for (size_t i = 0; i < output_tensor_buffers_2.size(); ++i) 
        {
            std::vector<float> logits;

            try 
            {
                int fp = std::any_cast<int>(output_tensors_2[i]->get_attr("fix_point"));
                float scale = std::pow(2.0f, -fp);

                auto tensor = output_tensor_buffers_2[i]->get_tensor();
                int size = tensor->get_element_num();
                logits.resize(size);

                auto data_ptr = reinterpret_cast<int8_t*>(output_tensor_buffers_2[i]->data().first);
            ...
            // Processar as logits para obter a classe predita
            auto [predicted_cid_2, predicted_prob_2] = postprocess_top1(logits);

            // Atualizar apenas se esta for a predição principal ou se for o primeiro output
            if (i == 0 || predicted_prob_2 > prob_2) 
            {
                pred_lbl_2 = predicted_cid_2;
                prob_2 = predicted_prob_2;
            }  
        }

        // Obter o nome da label
        if (pred_lbl_2 >= 0 && pred_lbl_2 < labels.size()) 
        {
            label_name_2 = labels[pred_lbl_2];
        }

        double postprocess_time_2 = get_timestamp() - t_start;
    ...
    graph.reset();     // free graph resources
...
/**
 * Two sequencial inference
 */
void TestB2(const std::vector<string>& image_paths,
            const std::vector<string>& labels,
            const std::unordered_map<std::string, int>& val_mapping,
            const std::string& results_csv)
{
    ...
    if (subgraphs.size() < 2) 
    {
        throw std::runtime_error("Need at least 2 DPU subgraphs for TestB2");
    }
    ...
    for (size_t i = 0; i < image_paths.size(); ++i) 
    {
        ...
        for (auto& input_buffer : input_tensor_buffers_1) 
        {
            ...
            auto data_ptr = input_buffer->data().first;
            size_t buffer_size = input_buffer->get_tensor()->get_data_size();
            std::memcpy((void*)data_ptr, blob_1.data, std::min(buffer_size, (size_t)(blob_1.total() * blob_1.elemSize())));
        }
        ...
        double inference_time_1 = get_timestamp() - t_start;
        ...
        for (size_t i = 0; i < output_tensor_buffers_1.size(); ++i) 
        {
            ...
            auto [cid, prob] = postprocess_top1(logits);
        ...
        double inference_time_2 = get_timestamp() - t_start;
...
{
    // Start power logger
    //Start_kv260_power_log(TEST_B1_POWER_RESULT_CSV_PATH);
    Start_kv260_power_log(TEST_B2_POWER_RESULT_CSV_PATH);
